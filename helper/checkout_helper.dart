...
class CheckOutHelper {
  ...
  static List<PaymentMethod> getActivePaymentList({required ConfigModel? configModel}){
    print('=== CHECKOUT HELPER DEBUG START ===');
    List<PaymentMethod> paymentMethodList = [];

    if (configModel == null) {
      print('DEBUG: configModel is NULL, returning empty list');
      print('=== CHECKOUT HELPER DEBUG END ===');
      return paymentMethodList;
    }

    // Add backend payment methods (including the original COD)
    if(configModel.activePaymentMethodList != null) {
      paymentMethodList.addAll(configModel.activePaymentMethodList!);
      print('DEBUG: Added ${configModel.activePaymentMethodList!.length} backend payment methods');
      print('DEBUG: Backend methods: ${configModel.activePaymentMethodList!.map((m) => '${m.getWay}/${m.getWayTitle}/${m.type}').join(', ')}');
    } else {
      print('DEBUG: configModel.activePaymentMethodList is NULL');
    }

    // Add offline payment methods (Bank Transfer, etc.) from SplashProvider
    try {
      final SplashProvider splashProvider = Provider.of<SplashProvider>(Get.context!, listen: false);
      if(splashProvider.offlinePaymentModelList != null && configModel.isOfflinePayment == true) {
        print('DEBUG: Found ${splashProvider.offlinePaymentModelList!.length} offline payment methods');
        for(var offlineMethod in splashProvider.offlinePaymentModelList!) {
          if(offlineMethod != null) {
            ...
            print('DEBUG: Added offline payment method: ${offlineMethod.methodName}');
          }
        }
      } else {
        print('DEBUG: Offline payment is disabled or no methods found');
        print('DEBUG: isOfflinePayment: ${configModel.isOfflinePayment}');
        print('DEBUG: offlinePaymentModelList: ${splashProvider.offlinePaymentModelList?.length ?? 0}');
      }
    } catch (e) {
      print('DEBUG: Error adding offline payment methods: $e');
    }

    // Force-include Telebirr regardless of backend toggle to restore previous behavior
    final bool hasTelebirr = paymentMethodList.any((m){
      final way = m.getWay?.toLowerCase() ?? '';
      final title = m.getWayTitle?.toLowerCase() ?? '';
      final type = m.type?.toLowerCase() ?? '';
      print('DEBUG: Checking existing method for Telebirr - way: "$way", title: "$title", type: "$type"');
      return way.contains('telebirr') || title.contains('telebirr') || type == 'telebirr';
    });

    print('DEBUG: hasTelebirr check result: $hasTelebirr');

    if(!hasTelebirr){
      final telebirrMethod = PaymentMethod(getWay: 'telebirr', getWayTitle: 'Telebirr', getWayImage: null, type: 'telebirr');
      paymentMethodList.add(telebirrMethod);
      print('DEBUG: Force-added Telebirr method: way=${telebirrMethod.getWay}, title=${telebirrMethod.getWayTitle}, type=${telebirrMethod.type}');
    } else {
      print('DEBUG: Telebirr already exists, not adding');
    }

    print('DEBUG: Final payment method list length: ${paymentMethodList.length}');
    print('DEBUG: Final methods: ${paymentMethodList.map((m) => '${m.getWay}/${m.getWayTitle}/${m.type}').join(', ')}');
    print('=== CHECKOUT HELPER DEBUG END ===');
  ...
  })
  {
    ...
    // Priority 1: Check if Amazon delivery option is selected
    if (orderProvider.isAmazonDeliveryEnabled && orderProvider.selectedDeliveryOption != null) {
      return orderProvider.selectedDeliveryCharge;
    }

    // Priority 2: Traditional delivery charge calculation

...
class ApiCheckerHelper {
  static void checkApi(ApiResponseModel apiResponse) {
    ...
    // Add proper null and empty checks to prevent RangeError crashes
    if(error.errors != null && error.errors!.isNotEmpty) {
      if(error.errors!.length > 0 && (error.errors![0].code == '401' || error.errors![0].code == 'auth-001') && 
        ...
        if(error.errors!.length > 0) {
          ...
          showCustomSnackBarHelper(getTranslated('something_went_wrong', Get.context!));
        }
      }
    } else {
      // Handle case where errors list is null or empty
      showCustomSnackBarHelper(getTranslated('something_went_wrong', Get.context!));
    }
  }
  ...
  static ErrorResponseModel getError(ApiResponseModel apiResponse){
    ...
    try{
      error = ErrorResponseModel.fromJson(apiResponse.response?.data != null ? apiResponse.response!.data : {});
    }catch(e){
      if(apiResponse.error != null){
        error = ErrorResponseModel.fromJson(apiResponse.error is Map ? apiResponse.error : {});
      }else{

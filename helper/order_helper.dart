...
class OrderHelper{
  ...
  static double getDeliveryCharge({required OrderModel? orderModel}) {
    // Prioritize Amazon delivery option charge if available
    if (orderModel?.deliveryOptionCharge != null && orderModel!.deliveryOptionCharge! > 0) {
      return orderModel.deliveryOptionCharge! + (orderModel.weightChargeAmount ?? 0);
    }

    // Fallback to legacy delivery charge system
    return (orderModel?.deliveryCharge ?? 0) + (orderModel?.weightChargeAmount ?? 0);
  }

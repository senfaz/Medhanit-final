        ...
        return orderProvider.isLoading && !ResponsiveHelper.isDesktop(context) ? Center(child: CustomLoaderWidget(color: Theme.of(context).primaryColor)) : SizedBox(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            CustomButtonWidget(
              ...
              onPressed: () async{
                if (kIsWeb) {
                  print('=== WEB PLACE ORDER DEBUG ===');
                  print('fromOfflinePayment: $fromOfflinePayment');
                  print('Starting validation checks...');
                }

                if(fromOfflinePayment){
                  if (kIsWeb) {
                    print('WEB: Inside fromOfflinePayment block - returning early');
                  }
                ...
                final OrderImageNoteProvider imageNoteProvider = Provider.of<OrderImageNoteProvider>(context, listen: false);

                if (kIsWeb) {
                  print('WEB: Starting validation chain...');
                }

                if(fromOfflinePayment && (orderProvider.getOfflinePaymentData()?.isEmpty ?? true)){
                  if (kIsWeb) print('WEB: Failed - offline payment data empty');
                  ...
                  if (kIsWeb) print('WEB: Failed - no delivery address');
                  ...
                  if (kIsWeb) print('WEB: Failed - no delivery area selected');
                  await scrollController?.animateTo(0, duration: const Duration(milliseconds: 100), curve: Curves.ease);
                  _openDropdown();
                }
                else if((orderProvider.selectedPaymentMethod == null ? (orderProvider.selectedOfflineValue == null) : orderProvider.selectedPaymentMethod!.getWay == null )){
                  if (kIsWeb) print('WEB: Failed - no payment method');
                  ...
                  if (kIsWeb) print('WEB: Failed - no delivery option selected');
                  ...
                  if (kIsWeb) print('WEB: Failed - delivery fee not set');
                  showCustomSnackBarHelper(getTranslated('delivery_fee_not_set_yet', context),isError: true);

                } else if(!CartHelper.isPrescriptionValid(Provider.of<CartProvider>(context, listen: false).cartList, imageNoteProvider.imageFiles)) {
                  if (kIsWeb) print('WEB: Failed - prescription validation');
                  showCustomSnackBarHelper(getTranslated('Please_upload_Prescription', context),isError: true);

                }else {
                  if (kIsWeb) print('WEB: All validations passed - proceeding with order');
                  ...
                  PlaceOrderModel placeOrderModel = PlaceOrderModel(
                    ...
                    timeSlotId: orderProvider.selectedDeliveryOption != null ? 0 : (orderProvider.timeSlots?.isNotEmpty == true ? orderProvider.timeSlots![orderProvider.selectTimeSlot].id : 0),
                  ...
                  if(orderProvider.selectedPaymentMethod?.getWay == 'cash_on_delivery' || orderProvider.selectedPaymentMethod?.getWay == 'wallet_payment' || orderProvider.selectedOfflineMethod != null) {
                    orderProvider.placeOrder(placeOrderModel, _callback);
                  } else {
                    String? hostname = html.window.location.hostname;
                    String protocol = html.window.location.protocol;
                    String port = html.window.location.port;
                    final String placeOrder =  convert.base64Url.encode(convert.utf8.encode(convert.jsonEncode(placeOrderModel.toJson())));
                    ...
                    orderProvider.clearPlaceOrder().then((_) => orderProvider.setPlaceOrder(convert.base64Encode(convert.utf8.encode(convert.jsonEncode(placeOrderModel.toJson())))).then((value) {

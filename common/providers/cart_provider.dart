...
class CartProvider extends ChangeNotifier {
  ...
  void addToCart(CartModel cartModel) {
    ...
    AnalyticsHelper.logAddToCart(
      currency: 'ETB',
      value: cartModel.discountedPrice! * cartModel.quantity!,
      items: [
        AnalyticsEventItem(
          itemId: cartModel.id.toString(),
          itemName: cartModel.name ?? '',
          price: cartModel.discountedPrice,
  ...
  void removeItemFromCart(int index, BuildContext context) {
    final removedItem = _cartList[index];
    _amount = _amount - (cartList[index].discountedPrice! * cartList[index].quantity!);

    // Track remove from cart event
    AnalyticsHelper.logRemoveFromCart(
      currency: 'ETB',
      value: removedItem.discountedPrice! * removedItem.quantity!,
      items: [
        AnalyticsEventItem(
          itemId: removedItem.id.toString(),
          itemName: removedItem.name ?? '',
          price: removedItem.discountedPrice,
